...
const MISSION_GROUPS = [
  {
    ...
    missions: [
      ...
      },
      {
        ...
        descKey: 'missionOmikujiGachaDrawDesc',
      },
      {
        ...
        descKey: 'missionOmikujiGachaEquipDesc',
      },
      {
        ...
        descKey: 'missionOmikujiAuctionListingDesc',
      },
      {
...
const i18n = {
  ja: {
    ...
    conditionOmikujiSpecificAch: (name) => `条件：原神おみくじの実績「${name}」を達成する`,
    ...
    missionOmikujiGachaDrawTitle: '裏面デザインガチャをひこう',
    missionOmikujiGachaDrawDesc: 'アカウント登録・ログインした状態で、原神おみくじの裏面デザインガチャを1回ひきます。',
    missionOmikujiGachaEquipTitle: '裏面を設定しよう',
  ...
  en: {
    ...
    conditionOmikujiSpecificAch: (name) => `Requirement: complete the Genshin Omikuji achievement "${name}"`,
    ...
    missionOmikujiGachaDrawTitle: 'Draw the card-back gacha',
    missionOmikujiGachaDrawDesc: 'While registered and logged in, draw the card-back design gacha once on Genshin Omikuji.',
    missionOmikujiGachaEquipTitle: 'Set a card back',
    missionOmikujiGachaEquipDesc: 'While registered and logged in, set a card-back design you got from the gacha on your fortune card.',
    missionOmikujiAuctionListingTitle: 'List something on the auction',
...
function isMissionAchievedFromData(mission, data) {
  if (mission.requiresOmikujiAchievementId) {
    const myAchievements = data.achievements || [];
    return myAchievements.includes(mission.requiresOmikujiAchievementId);
  }
...
function buildMissionCard(mission, siteUrl) {
  ...
  info.appendChild(desc);
  if (mission.requiresOmikujiAchievementId) {
    const targetAch = OMIKUJI_ACHIEVEMENTS.find((a) => a.id === mission.requiresOmikujiAchievementId);
    const targetName = targetAch ? (currentLang() === 'en' ? targetAch.nameEn : targetAch.name) : mission.requiresOmikujiAchievementId;
    const condition = document.createElement('p');
    condition.className = 'item-desc item-condition';
    condition.textContent = t.conditionOmikujiSpecificAch(targetName);
    info.appendChild(condition);
  }
  card.appendChild(info);
